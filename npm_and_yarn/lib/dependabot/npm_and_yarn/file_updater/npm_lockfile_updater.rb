    ...
    class FileUpdater
      class NpmLockfileUpdater
        ...
        def flattenend_manifest_dependencies
          ...
          package_json_content = updated_package_json_content(package_json)
        ...
        def post_process_npm_lockfile(updated_content)
          updated_content = replace_project_metadata(updated_content)
          ...
          updated_content = restore_packages_name(updated_content)

          # Switch back npm 7 lockfile "pacakages" requirements from the package.json
          updated_content = restore_locked_package_dependencies(updated_content)
        ...
        def restore_packages_name(updated_content)
          return updated_content unless npm7?

          parsed_original_lockfile = JSON.parse(lockfile.content)
          parsed_original_package = JSON.parse(package_json.content)
          parsed_updated_lockfile = JSON.parse(updated_content)
        ...
        # has finished.
        def restore_locked_package_dependencies(updated_content)
          return updated_content unless npm7?

          updated_package = updated_package_json_content(package_json)
          return updated_content unless updated_package

          parsed_lockfile = JSON.parse(updated_content)
          parsed_updated_package = JSON.parse(updated_package)
          dependency_names_to_restore = (dependencies.map(&:name) + git_dependencies_to_lock.keys).uniq

          NpmAndYarn::FileParser::DEPENDENCY_TYPES.each do |type|
            parsed_updated_package.fetch(type, {}).each do |dependency_name, original_requirement|
        ...
        def replace_swapped_git_ssh_requirements(updated_content)
          git_ssh_requirements_to_swap.each do |req|
        ...
        def replace_locked_git_dependencies(updated_content)
          # Switch from details back for git dependencies (they will have
          # changed because we locked them)
        ...
        def replace_tarball_urls(updated_content)
        ...
        def replace_project_metadata(updated_content)
          old_name = lockfile.content.match(/(?<="name": ").*(?=",)/)&.to_s
          updated_content = updated_content.sub(/(?<="name": ").*(?=",)/, old_name) if old_name
          updated_content
        end
